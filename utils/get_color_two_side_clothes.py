...
def get_surrounded_color(image, pt1,pt2):
    h,w,_ = image.shape
    if h/w < 2:
        circle_pixel_list = get_circle_pixel([w/2,h/2], w/8)
        circle_color_list = [image[pixel_color[1]][pixel_color[0]] for pixel_color in circle_pixel_list]
        main_color = get_colors(circle_color_list)
        return [main_color]
    else:
        main_color_list = []
        circle_pixel_list = get_circle_pixel([w/2,h/4], w/8)
        circle_color_list = [image[pixel_color[1]][pixel_color[0]] for pixel_color in circle_pixel_list]
        main_color = get_colors(circle_color_list)
        main_color_list.append(main_color)
        circle_pixel_list = get_circle_pixel([w/2,h*3/4], w/8)
        circle_color_list = [image[pixel_color[1]][pixel_color[0]] for pixel_color in circle_pixel_list]
        main_color = get_colors(circle_color_list)
        main_color_list.append(main_color)
