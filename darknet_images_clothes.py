...
def parser():
    ...
    parser.add_argument("--weights", default="yolo-clothes.weights",
...
def get_object_color(frame):
    ...
    model_k.fit(X)
    #multiple result
    # objects_rgb = np.array(model_k.cluster_centers_).astype(int)
    # hex_list = ['#%02x%02x%02x' % tuple(rgb) for rgb in objects_rgb]
    # return hex_list

    #single result
    objects_rgb = np.array(model_k.cluster_centers_[0]).astype(int)
    hex_list = '#%02x%02x%02x' % tuple(objects_rgb) 
    return hex_list

def get_color(a):
    colors, count = np.unique(a.reshape(-1,a.shape[-1]), axis=0, return_counts=True)
    bgr =  colors[count.argmax()]
    hex_list = '#%02x%02x%02x' % tuple([bgr[2],bgr[1],bgr[0]]) 
    return hex_list
...
def image_detection(image, network, class_names, class_colors, thresh):
    ...
    cv2.imwrite('image_color_recognize.jpg', image_resized)
    for label, acc, bbox in detections:
        x, y, w, h = bbox
        xmin, ymin, xmax, ymax = sizes2points(float(x), float(y), float(w), float(h))
        image_crop = image_resized[ymin:ymax, xmin:xmax]
        object_color = get_color(image_crop)
        print(object_color)
        #multiple color result
        # for object_color in objects_color:
        #     object_property_list.append({'type':label, 'colors': object_color})
        acc = int(float(acc))
        #single color result
        object_property_list.append({'type':label, 'proportion': acc, 'colors': [object_color]})
...
random.seed(3)  # deterministic bbox colors
network, class_names, class_colors = darknet.load_network('./data/yolo-obj.cfg', 
        './data/obj.data',
        'yolo-clothes.weights' ,
        batch_size=1)
# network, class_names, class_colors = darknet.load_network(
# args.config_file,
# args.data_file,
...
if __name__ == '__main__':
    server = HTTPServer(('0.0.0.0', 8002), MyServer)
